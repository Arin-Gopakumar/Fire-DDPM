...
def normalize_globally(x, stats):
    ...
    # This normalization is still applied per-channel on the 2D data before flattening
    ...
    for fd in tqdm(fires, desc=f"Processing {split} split"):
        for input_sid, imgs, mask_src, target_sid in collect_samples(fd, k):
            try:
                ...
                # Apply global normalization (still on C, H, W)
                x = normalize_globally(x, stats)

                # NEW: Iterate channel by channel and replace NaNs with 255
