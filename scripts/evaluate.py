...
def evaluate(config):
   ...
   for batch_idx, batch in enumerate(progress_bar):
       ...
       # targets now contain 0.0 and 255.0 values from prepare_data.py
       ...
       # --- NEW: Binarize targets for metrics ---
       ...
       targets_binary_for_metrics = (targets == 255.0).int()
      
       # pred_binary for metrics (still uses 0.5 threshold on model's probabilities)
       # This will be 1 for fire, 0 for no fire.
